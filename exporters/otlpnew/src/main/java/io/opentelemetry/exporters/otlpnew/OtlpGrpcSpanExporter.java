...
public final class OtlpGrpcSpanExporter implements SpanExporter {
  ...
  static {
    ...
    logger = Logger.getLogger(OtlpGrpcSpanExporter.class.getName());
  }
  ...
  public static OtlpGrpcSpanExporter.Builder newBuilder() {
    return new OtlpGrpcSpanExporter.Builder();
  }
  ...
  public CompletableResultCode export(final Collection<SpanData> spans) {
    ...
    TraceServiceGrpc.TraceServiceFutureStub exporter;
    if (this.deadlineMs > 0L) {
      exporter = this.traceService.withDeadlineAfter(this.deadlineMs, TimeUnit.MILLISECONDS);
    } else {
      exporter = this.traceService;
    }
    ...
    Futures.addCallback(
        ...
        new FutureCallback<ExportTraceServiceResponse>() {
          ...
          public void onFailure(Throwable t) {
            OtlpGrpcSpanExporter.logger.log(Level.WARNING, "Failed to export spans", t);
            result.fail();
          }
        },
  ...
  public static class Builder extends ConfigBuilder<OtlpGrpcSpanExporter.Builder> {
    ...
    public OtlpGrpcSpanExporter.Builder setChannel(ManagedChannel channel) {
    ...
    protected OtlpGrpcSpanExporter.Builder fromConfigMap(
        Map<String, String> configMap, ConfigBuilder.NamingConvention namingConvention) {
      configMap = namingConvention.normalize(configMap);
      Long value = getLongProperty("otel.otlp.span.timeout", configMap);
      if (value != null) {
        this.setDeadlineMs(value);
      }

      String endpointValue = getStringProperty("otel.otlp.endpoint", configMap);
      if (endpointValue != null) {
        this.setEndpoint(endpointValue);
      }

      Boolean useTlsValue = getBooleanProperty("otel.otlp.use.tls", configMap);
      if (useTlsValue != null) {
        this.setUseTls(useTlsValue);
      }

      String metadataValue = getStringProperty("otel.otlp.metadata", configMap);
